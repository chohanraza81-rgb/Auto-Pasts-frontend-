...
export default async function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  const related = await getPosts({ category: post.category, status: 'published' }).catch(() => []);
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 grid lg:grid-cols-[1fr_320px] gap-8">
      <article>
        ...
        <Schema data={{ '@type': 'Article', headline: post.title, description: post.metaDesc, author: { '@type': 'Person', name: 'Mike Johnson' }, datePublished: post.publishedAt }} />
      </article>
      <aside>
